...
4. Install **ffmpeg** on your PATH (required for browser WebM audio chunks).
...
6. In a new terminal, run the Next.js frontend:
```bash
cd web && npm install && npm run dev
```

Real-time coaching uses WebSocket `ws://localhost:8000/api/ws/session` (audio chunks, pose frames, final report on session end).
