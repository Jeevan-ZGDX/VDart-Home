...
class WhisperEngine:
    ...
    def transcribe_wav_data(self, wav_bytes: bytes, *, chunk_mode: bool = False) -> Dict[str, Any]:
            ...
            with io.BytesIO(wav_bytes) as f:
                with wave.open(f, 'rb') as wav:
                    ...
                    return self.transcribe(
                        audio_data,
                        beam_size=3 if chunk_mode else 5,
                        condition_on_previous_text=not chunk_mode,
                    )
        except Exception as e:
            return {"text": "", "error": f"WAV processing error: {str(e)}", "success": False}

    def transcribe_mp3_data(self, mp3_bytes: bytes, *, chunk_mode: bool = False) -> Dict[str, Any]:
    ...
    def transcribe_raw_audio(
        ...
        *,
        ...
        if file_type.lower() in ['mp3', 'm4a', 'ogg', 'flac', 'webm', 'mp4']:
            return self.transcribe_mp3_data(audio_bytes, chunk_mode=chunk_mode)
        else:
            return self.transcribe_wav_data(audio_bytes, chunk_mode=chunk_mode)
